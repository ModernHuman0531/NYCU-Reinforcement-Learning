...
class Policy(nn.Module):
    ...
    def calculate_loss(self, gamma=0.99):
        ...
        Advantage = returns - torch.tensor([saved_action.value.item() for saved_action in saved_actions])
        ...
        for i, saved_action in enumerate(saved_actions):
            ...
            policy_loss = -Advantage[i] * saved_action.log_prob
        ...
        loss = 0.1 * torch.stack(value_losses).sum() + torch.stack(policy_losses).sum()
        ########## END OF YOUR CODE ##########
...
if __name__ == '__main__':
    ...
    train(lr)
    test(f'LunarLander_{lr}.pth', env_name)

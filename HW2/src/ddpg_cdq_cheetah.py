...
class DDPG(object):
    def __init__(self, num_inputs, action_space, gamma=0.99, tau=0.001, hidden_size=256, lr_a=3e-4, lr_c=1e-3):
        ...
        # Add another critic for CDQ
        self.critic2 = Critic(hidden_size, self.num_inputs, self.action_space).to(device)
        self.critic_target2 = Critic(hidden_size, self.num_inputs, self.action_space).to(device)
    ...
    def update_parameters(self, batch):
        ...
        # Key part of CDQ: Add clipped noise on target action
        with torch.no_grad():
            # Why design the noise like this? Because we want to add noise to the target action to make it more robust, but we don't want the noise to be too large to push the action out of the valid range. So we use clipped noise with a certain standard deviation, which is a common practice in TD3 and CDQ. The paper suggests using clipped noise with std=0.2 for HalfCheetah-v5, so we can follow that.
            noise = torch.FloatTensor(action_batch.shape).normal_(0, 0.2).to(device) # Clipped noise with std=0.2, same as the paper
            noise = torch.clamp(noise, min=-0.5, max=0.5) # Clamp the noise to the valid range
            next_action_batch = (self.actor_target(next_state_batch)+noise).clamp(-1.0,1.0) # Add noise to the target action and clamp it into the valid range of action space.

            # Calculate the two target Q values from the two critic target networks
            next_state_action_values_1 = self.critic_target(next_state_batch, next_action_batch)
            next_state_action_values_2 = self.critic_target2(next_state_batch, next_action_batch)
        ...
        torch.nn.utils.clip_grad_norm_(self.critic.parameters(), 1.0) # Clip the gradients of the critic network to prevent exploding gradients, clip value is 1.0
        self.critic_optim.step()     # Update the critic network parameters

        # Update the second critic network for CDQ
        current_Q2 = self.critic2(state_batch, action_batch)
        # Compute the critic loss for the second critic networks
        value_loss2 = F.mse_loss(current_Q2,target_Q.detach())
        self.critic_optim2.zero_grad() # Clear the gradients of the second critic optimizer
        value_loss2.backward()
        torch.nn.utils.clip_grad_norm_(self.critic2.parameters(), 1.0) # Clip the gradients of the second critic network
    ...
    def save_model(self, env_name, suffix="", actor_path=None, critic_path=None):
        if not os.path.exists('preTrained_c/'):
            os.makedirs('preTrained_c/')
 
        if actor_path is None:
            actor_path = "preTrained_c/ddpg_cheetah_actor_{}_{}".format(env_name, suffix)
        if critic_path is None:
            critic_path = "preTrained_c/ddpg_cheetah_critic_{}_{}".format(env_name, suffix)
...
def train():    
    ...
    gamma = 0.99
    tau = 0.005               # Target tracking, tau=0.005
    ...
    lr_a = 2e-4               # actor lr for stability
    lr_c = 3e-4               # critic lr
...
if __name__ == '__main__':
    ...
    # train()
    test(actor_path='preTrained_c/ddpg_cheetah_actor_HalfCheetah-v5_ep499_score6756.8.pth', critic_path='preTrained_c/ddpg_cheetah_critic_HalfCheetah-v5_ep499_score6756.8.pth')

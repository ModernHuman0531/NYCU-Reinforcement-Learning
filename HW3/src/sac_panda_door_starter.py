...
class SACAgent:
    ...
    def update_model(self) -> Tuple[torch.Tensor, ...]:
        ...
        ########## Your Code (~10 lines)##########
        """
        In this code block, we need to :
        1. Compute the target Q value using the reward, discount factor, mask and the target V value of the next state.
        2. Compute the current Q values from qf_1 and qf_2
        3. Compute the MSE loss for qf_1 and qf_2 using the target Q value and current Q values.        
        """
        # Use torch.no_grad() to prevent gradients from being calculated for the target Q value
        with torch.no_grad():
            # Compute the target Q value
            target_q_value = reward + mask * self.gamma * self.vf_target(next_state)
        
        # Compute the current Q values from qf_1 and qf_2, and choose the minimum of the two Q values
        current_q_value_1 = self.qf_1(state, action)
        current_q_value_2 = self.qf_2(state, action)

        # Compute the MSE loss for qf_1 and qf_2 using the target Q value and current Q values
        qf_1_loss = F.mse_loss(current_q_value_1, target_q_value)
        qf_2_loss = F.mse_loss(current_q_value_2, target_q_value)
        ...
        """
        In this code block, we need to:
        1. Compute the current V value from vf
        2. Compute the target V value using the minimum of the two Q values and the log probability of the new action, weighted by alpha
        3. Compute the MSE loss for vf using the target V value and current V value
        """
        # Compute the current V value from vf
        current_v_value = self.vf(state)
        # Compute the target V value using the minimum of the two Q values and the log probability
        current_q_value = torch.min(current_q_value_1, current_q_value_2)
        target_v_value = current_q_value - alpha * log_prob

        # Compute the MSE loss for vf using the target V value and current V value
        # Use detach in target_v_value is to prevent gradient flow back to the actor and alpha when updating the critic
        vf_loss = F.mse_loss(current_v_value, target_v_value.detach())
        ...
        if self.total_step % self.policy_update_freq == 0:
            ...
            """
            In this code block, we need to:
            1. Compute the current Q value for the new action using qf_1 and qf_2, and choose the minimum of the two Q values
            2. Compute the actor loss using the current Q value, log probability of the new action, and alpha
            """
            # Compute the current Q value for the new action using qf_1 and qf_2, and choose the minimum of the two Q values
            new_q_value_1 = self.qf_1(state, new_action)
            # new_q_value_2 = self.qf_2(state, new_action)
            # new_q_value = torch.min(new_q_value_1, new_q_value_2)

            # Compute the actor loss using the current Q value, log probability of the new action, and alpha
            actor_loss = (alpha * log_prob - new_q_value_1).mean()
